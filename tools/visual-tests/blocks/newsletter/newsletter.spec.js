...
test.describe('Newsletter Visual Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Set default viewport size
    await page.setViewportSize({ width: 1280, height: 2000 });
  });

  test('newsletter visual test at Mobile viewport', async ({ page }) => {
    // Set viewport size
    await page.setViewportSize({ width: 320, height: 568 });

    // Navigate to the block variation
    await page.goto('/tools/sidekick/library.html?plugin=blocks&path=/tools/sidekick/blocks/newsletter&index=0&vtest=true');
    ...
    const block = await frame.waitForSelector('.newsletter', { timeout: 30000, state: 'visible' });
    ...
    if (!box) throw new Error('Could not get bounding box for Newsletter');
    ...
    const screenshotName = 'newsletter-0-Mobile.png';
  ...
  test('newsletter visual test at Tablet viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'newsletter-0-Tablet.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
  ...
  test('newsletter visual test at Desktop viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'newsletter-0-Desktop.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
  ...
  test('newsletter visual test at Large viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'newsletter-0-Large.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
