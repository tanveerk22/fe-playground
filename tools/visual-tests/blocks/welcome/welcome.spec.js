...
test.describe('Welcome Visual Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Set default viewport size
    await page.setViewportSize({ width: 1280, height: 2000 });
  });

  test('welcome visual test at Mobile viewport', async ({ page }) => {
    // Set viewport size
    await page.setViewportSize({ width: 320, height: 568 });

    // Navigate to the block variation
    await page.goto('/tools/sidekick/library.html?plugin=blocks&path=/tools/sidekick/blocks/welcome&index=0&vtest=true');
    ...
    const block = await frame.waitForSelector('.welcome', { timeout: 30000, state: 'visible' });
    ...
    if (!box) throw new Error('Could not get bounding box for Welcome');
    ...
    const screenshotName = 'welcome-0-Mobile.png';
  ...
  test('welcome visual test at Tablet viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'welcome-0-Tablet.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
  ...
  test('welcome visual test at Desktop viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'welcome-0-Desktop.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
  ...
  test('welcome visual test at Large viewport', async ({ page }) => {
    // Set viewport size
    ...
    // Take a screenshot of only the block area
    const screenshotName = 'welcome-0-Large.png';
    const screenshot = await page.screenshot({
      clip: box,
      timeout: 30000,
      animations: 'disabled',
